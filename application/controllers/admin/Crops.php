...
class Crops extends Brightery_Controller {

    public $layout = 'full';
    public $module = 'crops';
    public $model = 'Crops_model';
    ...
    public function index() {
        ...
        $this->{$this->model}->order_by['crops.name'] = 'ASC';
        $config['total_rows'] = $this->{$this->model}->get(TRUE);
        $config['suffix'] = '?' . http_build_query($_GET);
        $config['base_url'] = site_url('admin/crops/index');
    ...
    public function manage($id = null) {
        ...
        if ($op == 'add') {
            $this->form_validation->set_rules('name', 'Name', 'trim|required|is_unique[crops.name]');
        } else {
