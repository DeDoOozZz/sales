...
class Business_types extends Brightery_Controller {

    public $layout = 'full';
    public $module = 'business_types';
    public $model = 'Business_types_model';
    ...
    public function index() {
        ...
        $this->{$this->model}->order_by['business_types.name'] = 'ASC';
        $config['total_rows'] = $this->{$this->model}->get(TRUE);
        $config['suffix'] = '?' . http_build_query($_GET);
        $config['base_url'] = site_url('admin/business_types/index');
    ...
    public function manage($id = null) {
        ...
        if ($op == 'add') {
            $this->form_validation->set_rules('name', 'Name', 'trim|required|is_unique[business_types.name]');
        } else {
